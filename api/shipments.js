...
View your request:
${shipment.guestAccessToken ? `https://cubicship.com/request-status.html#${shipment.guestAccessToken}` : "https://cubicship.com/profile.html"}

Contact your counter to confirm any price, payment or pickup questions. Carrier tracking confirms delivery progress.`,
  });
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (labelId) {
      ...
      if (!canSeeShipment(user, shipment))
        return json(res, 403, {
          ok: false,
          error: "This label belongs to another location.",
        });
    ...
    const includeDeleted =
      user.role === "owner" && url.searchParams.get("includeDeleted") === "1";
    const shipments = await readShipments(user);
    const visibleShipments = shipments.filter(
      (shipment) => includeDeleted || !shipment.deletedAt,
    );
    return json(res, 200, {
      ok: true,
      shipments: sortNewestFirst(visibleShipments)
        .filter(
          (shipment) =>
            canSeeShipment(user, shipment) && matchesQuery(shipment, query),
        )
        .map(publicShipment),
    });
  }
  ...
  if (req.method === "PATCH") {
    ...
    if (!shipment)
      return json(res, 404, { ok: false, error: "Shipment was not found." });
    if (!canSeeShipment(user, shipment)) {
      return json(res, 403, {
        ok: false,
        error: "This ticket belongs to another location.",
      });
    }

    const hadTracking = Boolean(clean(shipment.carrierTracking));
    const previousStatus = shipment.status;
    if ("labelReference" in body)
      shipment.labelReference = clean(body.labelReference);
    if ("carrierTracking" in body)
      shipment.carrierTracking = clean(body.carrierTracking);
    ...
    if ("nextAction" in body || "priority" in body || "assignedTo" in body) {
      ...
      if ("nextAction" in body)
        shipment.staffWorkflow.nextAction = clean(body.nextAction);
      if ("priority" in body)
        shipment.staffWorkflow.priority = clean(body.priority || "normal");
      if ("assignedTo" in body)
        shipment.staffWorkflow.assignedTo = clean(body.assignedTo);
    }
    if (
      "paymentAmount" in body ||
      "paymentMethod" in body ||
      "paymentNotes" in body ||
      body.markPaymentDue ||
      body.markPaid
    ) {
      shipment.payment = shipment.payment || {};
      if ("paymentAmount" in body)
        shipment.payment.amount = clean(body.paymentAmount);
      if ("paymentMethod" in body)
        shipment.payment.method = clean(body.paymentMethod || "counter");
      if ("paymentNotes" in body)
        shipment.payment.notes = clean(body.paymentNotes);
      ...
      if (body.markPaid || body.verifyDropoff) {
        ...
        shipment.payment.paidAt =
          shipment.payment.paidAt || new Date().toISOString();
    ...
    if (
      "notaryStatus" in body ||
      "notaryAppointment" in body ||
      "notaryStaffNotes" in body ||
      "notaryIdentityStatus" in body
    ) {
      shipment.virtualNotary = shipment.virtualNotary || {};
      if ("notaryStatus" in body)
        shipment.virtualNotary.status = clean(
          body.notaryStatus || "request_received",
        );
      if ("notaryAppointment" in body)
        shipment.virtualNotary.appointmentConfirmedAt = clean(
          body.notaryAppointment,
        );
      if ("notaryStaffNotes" in body)
        shipment.virtualNotary.staffNotes = clean(body.notaryStaffNotes);
      if ("notaryIdentityStatus" in body)
        shipment.virtualNotary.identityStatus = clean(
          body.notaryIdentityStatus,
        );
      if (
        shipment.serviceType === "Virtual Notary" &&
    ...
    if (body.fileData) {
      ...
      if (!buffer.length || buffer.slice(0, 4).toString("utf8") !== "%PDF") {
        return json(res, 400, {
          ok: false,
          error: "The uploaded file must be a PDF label.",
        });
      }
      if (buffer.length > 4.5 * 1024 * 1024) {
        return json(res, 413, {
          ok: false,
          error: "Label PDF must be smaller than 4.5 MB.",
        });
      }
    ...
    if (requestedStatus) {
      if (!STATUSES.has(requestedStatus))
        return json(res, 400, {
          ok: false,
          error: "Shipment status is not valid.",
        });
      if (requestedStatus === "voided") {
        if (user.role !== "owner")
          return json(res, 403, {
            ok: false,
            error: "Owner access is required to void shipments.",
          });
        const reason = clean(
          body.voidReason ||
            body.deleteReason ||
            body.appendStaffNote ||
            body.staffNotes,
        );
        if (!reason)
          return json(res, 400, {
            ok: false,
            error: "Add a reason before voiding this shipment.",
          });
        shipment.voidedAt = shipment.voidedAt || new Date().toISOString();
        shipment.voidedBy = user.email;
        shipment.voidReason = reason;
      }
      if (!body.markPaymentDue && !body.verifyDropoff)
    ...
    shipment.auditLog = Array.isArray(shipment.auditLog)
      ? shipment.auditLog
      : [];
    shipment.auditLog.push({
      ...
      by: user.email,
      action:
        shipment.status === "voided" && statusChanged
          ? "shipment_voided"
          : body.appendStaffNote
            ? "staff_note_added"
            : body.verifyDropoff
      ...
      message:
        shipment.status === "voided" && statusChanged
          ? `Owner voided shipment. Reason: ${shipment.voidReason || "No reason recorded."}`
          : body.appendStaffNote
            ? "Staff added a timestamped note."
            : body.verifyDropoff
              ? "Staff confirmed customer drop-off/payment."
              : body.fileData
                ? "Staff uploaded a label PDF for customer download."
                : body.markPaid
                  ? "Staff marked manual/counter payment paid."
                  : body.markPaymentDue
                    ? "Staff marked manual/counter payment due."
                    : body.notifyCustomer
                      ? "Staff sent a customer update."
                      : statusChanged
                        ? `Staff changed shipment status from ${previousStatus || "blank"} to ${shipment.status}.`
                        : "Staff updated shipment workflow, tracking, or notes.",
    ...
    if (
      ...
      body.notifyCustomer ||
      body.fileData ||
      (!hadTracking && shipment.carrierTracking)
    ) {
      const notification = await sendCustomerUpdateEmail(shipment, user).catch(
        () => ({ ok: false }),
      );
      try {
        const latest = await readShipments(user),
          saved = latest.find((x) => x.id === shipment.id);
        if (saved) {
          saved.customerNotificationStatus = notification.ok
            ? "sent"
          ...
          await writeShipments(latest, user);
        }
      } catch {
        /* Keep committed staff changes if notification metadata conflicts. */
      }
    }
  ...
  if (req.method === "DELETE") {
    if (user.role !== "owner")
      return json(res, 403, {
        ok: false,
        error: "Owner access is required to delete or void shipments.",
      });
    const url = new URL(
      req.url,
      `https://${req.headers.host || "cubicship.com"}`,
    ...
    if (!id)
      return json(res, 400, { ok: false, error: "Shipment id is required." });
    if (!reason)
      return json(res, 400, {
        ok: false,
        error: "Add a reason before deleting or voiding this shipment.",
      });
    const shipments = await readShipments(user);
    const shipment = shipments.find((item) => item.id === id);
    if (!shipment)
    ...
    shipment.auditLog.push({
      ...
      by: user.email,
      action:
        mode === "delete" ? "shipment_deleted_from_queue" : "shipment_voided",
      message:
        mode === "delete"
          ? `Owner deleted shipment from active queue. Reason: ${reason}`
          : `Owner voided shipment. Reason: ${reason}`,
      previousStatus,
      nextStatus: "voided",
    });
    await writeShipments(shipments, user);
