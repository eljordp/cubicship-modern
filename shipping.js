(() => {
  ...
  const selectedLocation = () =>
    locations.find((x) => x.id === $("locationId").value);
  ...
  function updateMode() {
    ...
    form.querySelectorAll("[data-dropoff]").forEach((el) => {
      ...
      el.querySelectorAll("input").forEach((input) => {
        ...
        input.required =
          !quote &&
          (input.hasAttribute("data-dropoff-required") ||
            input.id === "senderCountry");
      });
    });
    $("contactHint").textContent = quote
      ? "We only need your contact details for a first quote. Staff may ask for more shipment details before confirming a price."
      : "Add the sender and receiver details so the counter can prepare your shipment.";
  }
  ...
  const dimensions = () =>
    ["length", "width", "height"]
      .filter((k) => value(k))
      .map((k) => `${k}: ${value(k)} ${value("sizeUnit")}`)
      .join(", ");
  function buildPayload() {
    ...
    return {
      ...
      notes: [
        `Request: ${isQuote() ? "Quote first" : "Prepare drop-off"}`,
        `Handoff: ${value("handoff") === "pickup-request" ? "Pickup requested — staff must confirm" : "Customer will bring items to counter"}`,
  ...
  function renderReview() {
    ...
    addReview(
      ...
      [
        ...
        [
          "Destination",
          [data.receiverCity, data.receiverPostal, data.receiverCountry]
            .filter(Boolean)
            .join(", "),
        ],
        [
    ...
    const entries = [
      [
        "Your contact",
        [data.senderName, data.senderEmail, data.senderPhone].join("\n"),
      ],
    ];
    if (!isQuote())
      entries.push(
  ...
  $("next").addEventListener("click", () => {
    if (fieldValid(steps[step])) showStep(step + 1);
  });
  $("back").addEventListener("click", () => showStep(step - 1));
  form.addEventListener("change", (event) => {
    if (event.target.name === "requestKind") updateMode();
    if (event.target.id === "locationId") updateBranch();
  });
  form.addEventListener("submit", async (event) => {
    ...
    if (!selectedLocation()) {
      showStep(0);
      fail("Choose an available counter.");
      return;
    }
    sending = true;
    ...
    const branch = selectedLocation(),
      data = buildPayload();
    try {
      const response = await fetch("/api/customer-shipments?source=qr", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result.ok || !result.shipment?.number)
        throw new Error(
          result.error ||
      ...
      $("receiptBranch").textContent = branch.name;
      $("receiptAddress").textContent = branch.address;
      $("receiptNext").textContent =
        data.handoff === "pickup-request"
          ? "Your pickup question has been sent to the counter. Call to confirm availability, price and timing before expecting a collection."
          : data.requestKind === "quote"
            ? "Your quote request has been sent to this counter for review. Staff must confirm the price and delivery estimate. If you have an urgent deadline, call the counter."
            : "Save this code and show it with your items at this counter. Staff will verify the details, confirm the price and help finish your shipment. Check current hours and the last collection before traveling.";
  ...
  $("copyReceipt").addEventListener("click", async () => {
    try {
      ...
      $("copyStatus").textContent = "Select and copy the code above.";
    }
  });
  const params = new URLSearchParams(location.search);
  if (params.get("mode") === "quote")
    form.querySelector("[name=requestKind][value=quote]").checked = true;
  updateMode();
  showStep(0, false);
  fetch("/assets/locations.json")
    ...
    .then((data) => {
      ...
      locations.forEach((branch) => {
        $("locationId").add(
          new Option(branch.city + ", " + branch.state, branch.id),
        );
      });
      const requested = params.get("location");
      if (requested && locations.some((x) => x.id === requested))
        $("locationId").value = requested;
      else if (requested)
        fail(
    ...
    .catch(() => {
      fail(
        "Locations could not load. Refresh this page or use the Locations link to call a counter.",
      );
      $("next").disabled = true;
    });
})();
